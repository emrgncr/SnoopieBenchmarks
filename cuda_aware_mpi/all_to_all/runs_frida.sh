...
mpirun --hostfile hostfile --map-by node --mca btl_openib_want_cuda_gdr 1 --mca pml ucx -x UCX_TLS=rc,sm,cuda_copy,gdr_copy -x LD_PRELOAD="$SANITIZER_PATH:/home/it4i-gencere/repos/ucx-snoop-v2/preload.so" -x UCX_PROTO_ENABLE=y -x UCX_PROTO_INFO=y -x UCX_LOG_PRINT_ENABLE=y -x UCX_LOG_LEVEL=data -np 16 ./frida_sep_run.sh
